...
const DynamicNode: React.FC<NodeProps> = ({ data, id }) => {
  ...
  const handleInputChange = useCallback(
    (inputName: string, value: string | number | boolean) => {
      setInputValues((prev) => {
  ...
  const isInputConnected = useCallback(
    (inputName: string) => {
      ...
      return edges.some(
        (edge: Edge) => edge.target === id && edge.targetHandle === handleId
      );
    },
    [edges, id]
  );
  ...
  const getInputValue = useCallback(
    (inputName: string) => {
      ...
      const inputConfig = nodeData.config.inputs.find(
